...
class Member extends Base {
    ...
    get bannable() {
        return this.manageable && this.guild.members.find((member) => member.id === this.client.user.id)?.permissions.has("banMembers");
    }
    ...
    get highestRole() {
        return this.getHighestRole(Array.from(this.roleCollection.values()));
    }

    get kickable() {
        return this.manageable && this.guild.members.find((member) => member.id === this.client.user.id)?.permissions.has("kickMembers");
    }

    get manageable() {
        if (this.user.id === this.guild.ownerID || this.user.id === this.client.user.id) return false;
        if (this.client.user.id === this.guild.ownerID) return true;
        const botPosition = this.getHighestRole(Array.from(this.guild.members.find((member) => member.id === this.client.user.id).roleCollection.values())).position;
        let member = this.getHighestRole(Array.from(this.roleCollection.values()))
        if (!member) return true;
        else member = member.position
        return botPosition > member;
    }
    ...
    get roleColor() {
        return this.getHighestRole(this.roleCollection.filter((x) => x.color !== 0));
    }
